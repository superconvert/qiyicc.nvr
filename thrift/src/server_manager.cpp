...
std::function<int(std::string &, Json::Value &)> verifyJson = [&] (std::string & data, Json::Value & json) -> int { 
	...
	nvr_shared<Json::CharReader> reader (builder.newCharReader()) ;
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
CServerManager::CServerManager() : m_bDemo(false)
{
...
void CServerManager::demo()
{
	...
	createDirectory ( getCameraPath ( camera ) .c_str () ) ;
...
int CServerManager::processCamera(Json::Value & json)
{
	...
	// 验证任务的合法性
	m_mutexTask .Lock () ;
	HashTaskObject::iterator taskIter = m_hashTask .begin () ;
	for ( ; taskIter != m_hashTask .end () ; )
	{
		...
		// 摄像头不存在
		m_mutexCameras .Lock () ;
		HashCameraObject::iterator cameraIter = m_hashCameras .find ( task .camera ) ;
...
std::string CServerManager::getCameraPath(CAMERAOBJECT & camera)
{
	return getRecorderPath () + "/" + std::to_string ( camera .id ) ;
}
...
int CServerManager::taskExit(pid_t pid)
{
	printLog ( "task (%d) exit begin", pid ) ;	
	m_mutexTask .Lock () ;
...
{
	m_mutexCameras .Lock () ;
	HashCameraObject::iterator iter = m_hashCameras .find ( id ) ;
	...
	m_mutexTask .Lock () ;
	HashTaskObject::iterator  taskIter = m_hashTask .find ( camera .id ) ;
...
{	
	...
	else if ( 0 == pid )
	{
		...
		std::string childLog = m_workPath + "/child.log" ;
